#!/usr/bin/env node
// test/helpers/sw-version.mjs — the client version has to be ABOVE main's, not just
...
// merged after, so main went BACKWARDS from v17 to v16 and #84 existed only to put it
...
// leaves main at v16 does not fail on the phone, it does NOTHING on the phone, for every
...
//   2. this commit is already in main   -> skip. On main itself, or on a branch that has landed,
//                                         there is nothing to be ahead of. Not a pass: a pass
//                                         would claim an ordering nobody checked.
//   3. no origin/main to read           -> skip locally, FAIL in CI on a pull request. A guard
//                                         that quietly no-ops in the one place it matters reads
//                                         as covered and is not.
//   4. above main                       -> pass.
//   5. equal to main, identical bytes   -> pass. The overwhelming majority of PRs never touch
...
export function verdict(s) {
  ...
  if (s.headInMain) return { kind: 'skip', reason: 'this commit is already in main — nothing to be ahead of' };
  if (!s.refPresent) {
    return s.prCI
      ? { kind: 'fail', reason: 'CI has no origin/main, so the ordering was never checked — the fetch step in .github/workflows/test.yml did not produce it' }
      : { kind: 'skip', reason: 'no origin/main in this clone — `git fetch origin main` to enable the ordering check' };
  }
  if (theirs == null) return { kind: 'fail', reason: 'origin/main:web/sw.js has no VERSION to compare against' };
  if (ours > theirs) return { kind: 'pass', reason: `v${ours} is above main's v${theirs}` };
  if (ours === theirs && s.ourHash != null && s.ourHash === s.theirHash) {
    return { kind: 'pass', reason: `v${ours} matches main and so does the client, so there is nothing to move for` };
  }
  if (ours === theirs) {
    return { kind: 'fail', reason: `v${ours} is main's number with different bytes — somebody else already took it; go to v${theirs + 1}` };
  }
  return { kind: 'fail', reason: `v${ours} is BELOW main's v${theirs} — a phone holding v${theirs} would never load it; go to v${theirs + 1}` };
}
...
const base = { refPresent: true, headInMain: false, prCI: false, ...H };
const kindOf = (o) => verdict({ ...base, ...o }).kind;

is('main+1 is what a branch should be', 'pass', kindOf({ ours: 20, theirs: 19 }));
// THE THREE FAILURES ARE THE POINT OF THE FILE.
is('...main\'s own number is not', 'fail', kindOf({ ours: 19, theirs: 19 }));
is('...and below main is not (#81 after #83)', 'fail', kindOf({ ours: 16, theirs: 17 }));
is('...nor is main-minus-one with a bump in it', 'fail', kindOf({ ours: 18, theirs: 19 }));
...
// The reason text carries the number to use, because "go and look up main's" is the step
...
is('an untouched client may keep main\'s number', 'pass',
...
// ...and the exemption is for EQUAL only. Below main is wrong even with nothing to deploy,
// because the number is the whole of what a phone compares.
is('...and never excuses a version below main', 'fail',
   kindOf({ ours: 18, theirs: 19, ourHash: 'cccccccccccc', theirHash: 'cccccccccccc' }));

// The three states that are not a comparison at all.
is('on main itself there is nothing to compare', 'skip', kindOf({ ours: 19, theirs: 19, headInMain: true }));
is('...and a merged branch is the same case', 'skip', kindOf({ ours: 19, theirs: 20, headInMain: true }));
...
is('a clone with no origin/main skips', 'skip', kindOf({ ours: 20, theirs: null, refPresent: false }));
...
// main it answers yes, on a branch that has already landed it answers yes, and on anything
...
// a branch whose history does not reach main is a branch that has not landed.
const ok = (...args) => {
  const r = spawnSync('git', ['-C', ROOT, ...args], { stdio: 'ignore' });
  return r.status === 0;
};
const headInMain = refPresent && ok('merge-base', '--is-ancestor', 'HEAD', REF);
...
if (v.kind === 'skip') skipGroup('the client version is above main\'s', v.reason);
else is(`the client version is above main's — ${v.reason}`, 'pass', v.kind);
