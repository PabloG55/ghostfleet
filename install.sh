...
mkdir -p "$BIN_DIR"
for b in claude-fleet claude-here cf-sync fleet-schedule fleet-send fleet-list fleet-read fleet-spawn fleet-jump fleet-pause fleet-resume fleet-governor fleet-statusbar fleet-worktrees fleet-answer fleet-inbox fleet-stop fleet-scratch fleet-companion fleet-clean; do
  ln -sf "$FLEET_HOME/bin/$b" "$BIN_DIR/$b"
done
echo "✓ linked claude-fleet + helpers (here, cf-sync, schedule, send, list, read, spawn, jump, pause, resume, governor, statusbar, worktrees, answer, inbox, stop, scratch, companion, clean) -> $BIN_DIR"
