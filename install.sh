...
mkdir -p "$BIN_DIR"
for b in ghostfleet claude-here cf-sync fleet-schedule fleet-send fleet-list fleet-read fleet-spawn fleet-jump fleet-pause fleet-resume fleet-governor fleet-statusbar fleet-worktrees fleet-answer fleet-inbox fleet-stop fleet-scratch fleet-companion fleet-clean fleet-open fleet-project fleet-adopt; do
  ln -sf "$FLEET_HOME/bin/$b" "$BIN_DIR/$b"
done
ln -sf "$FLEET_HOME/bin/ghostfleet" "$BIN_DIR/claude-fleet"   # back-compat: the old entry point
echo "✓ linked ghostfleet + helpers (here, cf-sync, schedule, send, list, read, spawn, jump, pause, resume, governor, statusbar, worktrees, answer, inbox, stop, scratch, companion, clean, open, project, adopt) -> $BIN_DIR"
